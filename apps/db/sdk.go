...
func SAPaint_Text(x, y, w, h float64, style *_SA_Style, value string, selection, edit, enable bool) bool {
	...
	err := style.Register()
	if err == nil {
		ret = _sa_paint_text(x, y, w, h, style.Id, _SA_stringToPtr(value),
			_SA_boolToUint32(selection), _SA_boolToUint32(edit), _SA_boolToUint32(enable)) > 0
	}
	return ret
}

func SAPaint_TextWidth(style *_SA_Style, value string, cursorPos int) float64 {
	return _sa_paint_textWidth(style.Id, _SA_stringToPtr(value), int64(cursorPos))
}
...
func (b *_SA_Progress) Show(x, y, w, h int) {
	...
	if SA_DivStart(x, y, w, h) {
		...
		err := b.styleFrame.Register()
		if err == nil {
			err = b.styleStatus.Register()
			if err == nil {
...
func (b *_SA_Slider) Show(x, y, w, h int) _SA_SliderOut {
	...
	if SA_DivStart(x, y, w, h) {
		...
		err := b.styleTrack.Register()
		if err == nil {
			err = b.styleThumb.Register()
			if err == nil {
...
func (b *_SA_Checkbox) Show(x, y, w, h int) bool {
	...
	if SA_DivStart(x, y, w, h) {
		...
		err := b.styleCheck.Register()
		if err == nil {
			err = b.styleLabel.Register()
			if err == nil {
