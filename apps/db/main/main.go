...
func TablesList() {
	...
	for x, table := range store.Tables {
		SA_DivStart(x, 0, 1, 1)
		{
			...
			if SA_ButtonMenu(table.Name).Icon(SA_FileFromResources("", "table.png"), 0.13).Pressed(isSelected).Show(0, 0, 1, 1).click {
...
func ColumnDetail(table *Table, column *Column) {
	...
	SA_DivStart(0, 0, 1, 1)
	{
		...
		//convert type
		if SA_ButtonStyle(column.GetColumnName(), &g_ButtonLeft).Icon(SA_FileFromResources("", _getColumnIcon(column.Type, column.Render)), 0.13).Enable(!IsBlob(column.Type)).Show(1, 0, 1, 1).click {
		...
		if SA_DialogStart("changeType") {
			...
			if IsText(column.Type) {
				...
				if SA_ButtonMenu(trns.INTEGER).Icon(SA_FileFromResources("", _getColumnIcon("INT", "")), 0.13).Enable(column.Render != "PERCENT").Show(0, y, 1, 1).click {
					column.Convert(table, "INT", "")
				}
				y++

				if SA_ButtonMenu(trns.REAL).Icon(SA_FileFromResources("", _getColumnIcon("REAL", "")), 0.13).Enable(column.Render != "PERCENT").Show(0, y, 1, 1).click {
				...
				if SA_ButtonMenu(trns.REAL).Icon(SA_FileFromResources("", _getColumnIcon(column.Type, "")), 0.13).Enable(column.Render != "").Show(0, y, 1, 1).click {
					column.Render = ""
				}
				y++

				if SA_ButtonMenu(trns.PERCENT).Icon(SA_FileFromResources("", _getColumnIcon(column.Type, "PERCENT")), 0.13).Enable(column.Render != "PERCENT").Show(0, y, 1, 1).click {
				...
				if SA_ButtonMenu(trns.TEXT).Icon(SA_FileFromResources("", _getColumnIcon("TEXT", "")), 0.13).Enable(column.Render != "PERCENT").Show(0, y, 1, 1).click {
				...
				if SA_ButtonMenu(trns.INTEGER).Icon(SA_FileFromResources("", _getColumnIcon(column.Type, "")), 0.13).Enable(column.Render != "").Show(0, y, 1, 1).click {
					column.Render = ""
				}
				y++

				if SA_ButtonMenu(trns.CHECK_BOX).Icon(SA_FileFromResources("", _getColumnIcon(column.Type, "CHECK_BOX")), 0.13).Enable(column.Render != "CHECK_BOX").Show(0, y, 1, 1).click {
					column.Render = "CHECK_BOX"
				}
				y++

				if SA_ButtonMenu(trns.DATE).Icon(SA_FileFromResources("", _getColumnIcon(column.Type, "DATE")), 0.13).Enable(column.Render != "DATE").Show(0, y, 1, 1).click {
					column.Render = "DATE"
				}
				y++

				if SA_ButtonMenu(trns.RATING).Icon(SA_FileFromResources("", _getColumnIcon(column.Type, "RATING")), 0.13).Enable(column.Render != "RATING").Show(0, y, 1, 1).click {
	...
	SA_DivStart(0, 2, 1, 1)
	{
		...
		//filter
		if SA_ButtonStyle(trns.FILTER, &g_ButtonLightLeft).Icon(SA_FileFromResources("", "filter.png"), 0.13).Show(0, 0, 1, 1).click {
		...
		if SA_ButtonStyle(trns.SORT, &g_ButtonLightLeft).Icon(SA_FileFromResources("", "sort_az.png"), 0.13).Enable(sort_notUse).Show(1, 0, 1, 1).click {
			table.Sort.Add(column.Name, 0)
		}

		//sort
		if SA_ButtonStyle(trns.SORT, &g_ButtonLightLeft).Icon(SA_FileFromResources("", "sort_za.png"), 0.13).Enable(sort_notUse).Show(2, 0, 1, 1).click {
...
func TableColumns(table *Table) {
	...
	for _, col := range table.Columns {
		...
		SA_DivStart(x, 0, 1, 1)
		{
			...
			if col.isRowId() {
				...
				if SA_ButtonStyle(nm, &g_ButtonColumnHeader).Icon(SA_FileFromResources("", _getColumnIcon(col.Type, col.Render)), 0.13).Show(0, 0, 1, 1).click && !col.isRowId() {
	...
	if SA_DialogStart("createColumn") {
		...
		//types
		if SA_ButtonMenu(trns.TEXT).Icon(SA_FileFromResources("", _getColumnIcon("TEXT", "")), 0.13).Enable(err == nil).Show(0, y, 1, 1).click {
			add_type = "TEXT"
		}
		y++

		if SA_ButtonMenu(trns.INTEGER).Icon(SA_FileFromResources("", _getColumnIcon("INT", "")), 0.13).Enable(err == nil).Show(0, y, 1, 1).click {
			add_type = "INT"
		}
		y++

		if SA_ButtonMenu(trns.REAL).Icon(SA_FileFromResources("", _getColumnIcon("REAL", "")), 0.13).Enable(err == nil).Show(0, y, 1, 1).click {
			add_type = "REAL"
		}
		y++

		if SA_ButtonMenu(trns.BLOB).Icon(SA_FileFromResources("", _getColumnIcon("BLOB", "")), 0.13).Enable(err == nil).Show(0, y, 1, 1).click {
			add_type = "BLOB"
		}
		y++

		if SA_ButtonMenu(trns.CHECK_BOX).Icon(SA_FileFromResources("", _getColumnIcon("INT", "CHECK_BOX")), 0.13).Enable(err == nil).Show(0, y, 1, 1).click {
			add_type = "INT"
			render = "CHECK_BOX"
		}
		y++

		if SA_ButtonMenu(trns.DATE).Icon(SA_FileFromResources("", _getColumnIcon("INT", "DATE")), 0.13).Enable(err == nil).Show(0, y, 1, 1).click {
			add_type = "INT"
			render = "DATE"
		}
		y++

		if SA_ButtonMenu(trns.PERCENT).Icon(SA_FileFromResources("", _getColumnIcon("REAL", "PERCENT")), 0.13).Enable(err == nil).Show(0, y, 1, 1).click {
			add_type = "REAL"
			render = "PERCENT"
		}
		y++

		if SA_ButtonMenu(trns.RATING).Icon(SA_FileFromResources("", _getColumnIcon("INT", "RATING")), 0.13).Enable(err == nil).Show(0, y, 1, 1).click {
			add_type = "INT"
			render = "RATING"
				...
				for _, col := range table.Columns {
					...
					if col.isRowId() {
						...
						if r > 0 && err == nil {
							...
							res := SA_FileGetBlob("", table.Name, col.Name, r)
						...
						case "RATING":
							if SA_DivStart(x, 0, 1, rowSize) {
								...
								act, writeCell = SA_Rating(act, col.Prop_rating_max_stars, SA_ThemeCd(), SA_ThemeGrey(0.8), SA_FileFromResources("", "star.png"))
								if writeCell {
