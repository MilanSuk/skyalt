...
#### Create
1) open the main Menu(SkyAlt logo) and under "Developers" click "Create app".
2) set the name of the app and the programming language. Click the "Create App" button.

*The new app is created inside `/apps` folder.*

#### Debug
1) in SkyAlt, add your new app under some database(SkyAlt will complain that 'main.wasm' is missing, ignore it).
2) open VSCode. From the top menu select "File" -> "Open Folder" and select your app folder. Run the app with the F5 key.
3) in SkyAlt, the app will show up with a blue border(that means it's in debug mode).
...
#### Package
1) open the main Menu(SkyAlt logo) and under "Developers" click "Package app".
2) select the app. Click the 'Package app' button.
