...
func (arr *UiLayoutArray) SetFills(childs []*Layout, cols bool) {
	arr.fills = make([]bool, arr.NumInputs())

	for i := range arr.fills {
		for _, it := range childs {
			if it.IsShown() {
				if (cols && i >= it.X && i < it.X+it.W) ||
					(!cols && i >= it.Y && i < it.Y+it.H) {
					arr.fills[i] = true
					break
				}
			}
...
func (arr *UiLayoutArray) Update(cell int, window int) {
	...
	arr.outputs = make([]int, arr.NumInputs())
	...
	for i := range arr.inputs {
		...
		if arr.inputs[i].resize == nil {
			...
			arr.inputs[i].min = resV
			arr.inputs[i].max = resV

			arr.inputs[i].resize.value = resV
		}
	}
...
func (arr *UiLayoutArray) GetSumOutput(i int) int {
