...
var g_theme_light = WinCdPalette{
...
func Layout_Cell() int { //number of pixels in one cell
	return int(float32(OpenFile_DeviceSettings().Dpi) / 2.5)
}

func Paint_GetPalette() *WinCdPalette {

	env := OpenFile_DeviceSettings()
...
func Layout_GetDateFormat() string {
	return OpenFile_DeviceSettings().DateFormat
}

func Layout_GetDatePage() *int64 {
	page := &OpenFile_DeviceSettings().DatePage
	if *page == 0 {
		*page = time.Now().Unix()
	}
	return page
...
func Layout_WriteError(err error) error {
	...
	fmt.Println("Layout_WriteError:", err.Error())
...
func Layout_ConvertTextDate(unix_sec int64) string {
	...
	switch OpenFile_DeviceSettings().DateFormat {
	case "eu":
