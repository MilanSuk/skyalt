...
func (st *DatePicker) Build(layout *Layout) {
	...
	Today.Tooltip = Layout_ConvertTextDate(time.Now().Unix())
...
func Date_GetMonthYear(unix_sec int64) string {
	tm := time.Unix(unix_sec, 0)
	return Layout_GetMonthText(int(tm.Month())) + " " + strconv.Itoa(tm.Year())
}
