...
func (st *Calendar) Build(layout *Layout) {
	...
	if format == "us" {
		//"us"
		txt := layout.AddText(0, 0, 1, 1, Layout_GetDayTextShort(7))
		txt.Align_h = 1
		for x := 1; x < 7; x++ {
			txt := layout.AddText(x, 0, 1, 1, Layout_GetDayTextShort(x))
			txt.Align_h = 1
		}
	} else {
		for x := 1; x < 8; x++ {
			txt := layout.AddText(x-1, 0, 1, 1, Layout_GetDayTextShort(x))
			txt.Align_h = 1
		}
	}
