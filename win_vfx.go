...
func (ptcs *WinParticles) Tick(cd_orig color.RGBA, depth int, win *Win) bool {
	...
	win.DrawRect(OsV2{0, 0}, win.GetScreenCoord().Size, depth, color.RGBA{255, 255, 255, 200})
...
func (ptcs *WinParticles) Draw(cd_theme color.RGBA, depth int, win *Win) (bool, error) {
	...
	for i := 0; i < ptcs.num; i++ {
		...
		if ptcs.alphas[i] > 0 && a > 0.01 && !last_p.Cmp(ptcs.poses[i].toV2()) { // one particles per pixel(in row)
			...
			cdd := cd
			cdd.A = uint8(a * 255)
			win.DrawPointCdF(p, depth+1, cdd)
	...
	if ptcs.cell > 0 {
		...
		cq.Size.Y = ptcs.cell
		if title != "" {
			win.DrawText(ptcs.description, InitWinFontPropsDef(ptcs.cell), cd, cq, depth+2, OsV2{0, 1}, 0, 1)
		}
		if proc != "" {
