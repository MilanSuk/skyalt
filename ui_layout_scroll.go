...
func (scroll *UiLayoutScroll) _UpdateV(view OsV4, ui *Ui) OsV4 {
	...
	if scroll.data_height <= scroll.screen_height {
		...
		outSlider.Start.Y = view.Start.Y + int(float32(view.Size.Y)*(float32(scroll.GetWheel())/float32(scroll.data_height)))

		outSlider.Size.X = scroll._GetWidth(ui)
		outSlider.Size.Y = int(float32(view.Size.Y) * (float32(scroll.screen_height) / float32(scroll.data_height)))
	}
	return outSlider
}
...
func (scroll *UiLayoutScroll) DrawV(view OsV4, showBackground bool, ui *Ui) {
	...
	ui.GetWin().buff.AddRect(slider, cd, 0)
}
...
func (scroll *UiLayoutScroll) IsMove(dom *Layout3, wheel_add int, deep int, ver bool) bool {
...
func (scroll *UiLayoutScroll) TouchV(dom *Layout3) bool {
	backup_wheel := scroll.wheel
	ui := dom.ui

	canUp := scroll.IsMove(dom, -1, 0, true)
	canDown := scroll.IsMove(dom, +1, 0, true)
	...
	scrollCoord := dom.scrollV.GetScrollBackCoordV(dom.view, ui)

	sliderFront := scroll._UpdateV(scrollCoord, ui)
	midSlider := sliderFront.Size.Y / 2

	isTouched := dom.GetUis().touch.IsFnMove(0, dom.props.Hash, 0, 0)
	...
	if isTouched { // click on slider
		mid := float32((ui.GetWin().io.Touch.Pos.Y - scrollCoord.Start.Y) - midSlider - scroll.clickRel)
		...
		mid := float32((ui.GetWin().io.Touch.Pos.Y - scrollCoord.Start.Y) - midSlider)
		scroll.SetWheel(int((mid / float32(scrollCoord.Size.Y)) * float32(scroll.data_height)))
	...
	if isTouched {
		dom.GetUis().touch.Set(0, dom.props.Hash, 0, 0)
	}
...
func (scroll *UiLayoutScroll) TouchH(needShiftWheel bool, dom *Layout3) bool {
	backup_wheel := scroll.wheel
	ui := dom.ui

	canLeft := scroll.IsMove(dom, -1, 0, !ui.GetWin().io.Keys.Shift)
	canRight := scroll.IsMove(dom, +1, 0, !ui.GetWin().io.Keys.Shift)
	...
	scrollCoord := dom.scrollV.GetScrollBackCoordH(dom.view, ui)

	sliderFront := scroll._UpdateH(scrollCoord.Start, ui)
	midSlider := sliderFront.Size.X / 2

	isTouched := dom.GetUis().touch.IsFnMove(0, 0, dom.props.Hash, 0)
	if ui.GetWin().io.Touch.Start {
		isTouched = sliderFront.Inside(ui.GetWin().io.Touch.Pos)
	...
	if isTouched { // click on slider
		mid := float32((ui.GetWin().io.Touch.Pos.X - scrollCoord.Start.X) - midSlider - scroll.clickRel)
		scroll.SetWheel(int((mid / float32(scroll.screen_height)) * float32(scroll.data_height)))
	} else if ui.GetWin().io.Touch.Start && scrollCoord.Inside(ui.GetWin().io.Touch.Pos) && !sliderFront.Inside(ui.GetWin().io.Touch.Pos) { // click(once) on background
		mid := float32((ui.GetWin().io.Touch.Pos.X - scrollCoord.Start.X) - midSlider)
		scroll.SetWheel(int((mid / float32(scroll.screen_height)) * float32(scroll.data_height)))
	...
	if isTouched {
		dom.GetUis().touch.Set(0, 0, dom.props.Hash, 0)
	}
