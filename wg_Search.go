...
func (st *Search) Build(layout *Layout) {
	...
	layout.AddImageCd(0, 0, 1, 1, "resources/search.png", nil, color.RGBA{0, 0, 0, 255}).Margin = 0.2
...
func (st *Search) Draw(rect Rect, layout *Layout) (paint LayoutPaint) {
	if st.Value != nil && *st.Value != "" {
		...
		rc = rc.CutRight(1)
		rc = rc.Cut(0.06)
		paint.RectRad(rc, backCd, backCd, backCd, 0, layout.ui.router.services.sync.GetRounding())
	}
	return
}
